...
from br.models import Subscription
...
def compute_nonreporting(subscription, end_of_month=False):
    ...
    for location in subscription.locations.filter(type__name__in=['Country', 'State']):
        ...
        for loc in get_nonperforming_locations(location, start_date, end_date):
            nonreporting_locations.append({'name': loc, 'type': report_region })

    return {'report_region': report_region, 'non_reporting': nonreporting_locations, 'today': datetime.now()}
...
def mid_month_report(subscription_id):
    ...
    if user.email:
        send_mail(email_subject, u'', settings.DEFAULT_FROM_EMAIL, [user.email], html_message=email_body)
