...
DR_LOCATION_TYPE_NAMES = ['country', 'state', 'lga']


def _default_queryset():
    queryset = Location.objects.filter(
            type__name__iregex='({})'.format('|'.join(DR_LOCATION_TYPE_NAMES))
        ).order_by('lft', 'code')
...
@api_view()
def dr_location_types(request):
    return JsonResponse({
        'types': DR_LOCATION_TYPE_NAMES
    })
...
class DeathReportListCreateView(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        return super(DeathReportListCreateView, self).get(request, *args, **kwargs)
    ...
    def get_queryset(self):
        ...
        queryset = super(DeathReportListCreateView, self).get_queryset()
        if year is None:
