...
class Reporter(models.Model):
    ...
    def exists(klass, reporter, connection):
        ...
        try:
            ...
            existing_conn = PersistantConnection.objects.get(backend=connection.backend,\
...
class PersistantBackend(models.Model):
    ...
    def from_message(klass, msg):
        ...
        be_slug = msg.connections[0].backend.name
        try:
            be = klass.objects.get(slug=be_slug)
        except klass.DoesNotExist:
            be = klass.objects.create(slug=be_slug, title=be_slug)
        return be
